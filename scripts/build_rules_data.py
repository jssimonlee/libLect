...
def read_generated_entries(path: Path) -> list[dict]:
    text = path.read_text(encoding="utf-8")
    marker = "window.LIBRARY_KNOWLEDGE = "
    if marker not in text:
        raise ValueError(f"Could not find generated data marker in {path}")
    payload = text.split(marker, 1)[1].strip()
    if not payload.endswith(";"):
        raise ValueError(f"Generated data in {path} does not end with a semicolon")
    entries = json.loads(payload[:-1])
    if not isinstance(entries, list):
        raise ValueError(f"Generated data in {path} must be a JSON array")
    return entries
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Reuse regulation records in the existing output instead of downloading and parsing the PDF",
    )
    parser.add_argument(
        "--preserve-curated-versions",
        action="store_true",
        help="Keep existing guide-* verification dates during an incremental library-site refresh",
    )
    args = parser.parse_args()

    previous_entries = read_generated_entries(args.output) if (
        args.output.exists() and (args.reuse_existing_regulations or args.preserve_curated_versions)
    ) else []
    if args.reuse_existing_regulations:
        regulations = [entry for entry in previous_entries if entry.get("sourceType") == "regulation"]
        if not regulations:
            raise RuntimeError("No existing regulation records were found to reuse")
    else:
        pdf_path = args.pdf.resolve() if args.pdf else download_pdf()
        if not pdf_path.exists():
            raise FileNotFoundError(pdf_path)
        regulations = regulation_entries(pdf_path)
    ...
    entries = guide_entries(checked_on) + library_entries(checked_on) + regulations + law_entries()
    if args.preserve_curated_versions:
        previous_versions = {
            entry.get("id"): entry.get("version") for entry in previous_entries
            if str(entry.get("id", "")).startswith("guide-")
        }
        for entry in entries:
            if entry.get("id") in previous_versions:
                entry["version"] = previous_versions[entry["id"]]
